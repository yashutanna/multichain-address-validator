"use strict";
// const BTCValidator = require("../src/bitcoin_validator");
// const ETHValidator = require("../src/ethereum_validator");
// const Base58Validator = require("../src/base58_validator");
// const TRXValidator = require("../src/tron_validator");
...
// expect = chai.expect
const wallet_address_validator_1 = require("../src/wallet_address_validator");
// const addresses = require('./addresses/addresses')
// @ts-ignore
const addresses_1 = __importDefault(require("./addresses/addresses"));
// const WAValidator = isNode ? require('../src/wallet_address_validator') : (window as any).WAValidator
function valid(address, chain) {
    const valid = (0, wallet_address_validator_1.validate)(address, chain);
    (0, chai_1.expect)({ address, chain, valid }).to.deep.equal({ address, chain, valid: true });
}
function invalid(address, chain) {
    const valid = (0, wallet_address_validator_1.validate)(address, chain);
    (0, chai_1.expect)({ address, chain, valid }).to.deep.equal({ address, chain, valid: false });
}
const TestCases = {
    'bitcoin': {
...
(0, mocha_1.describe)('WAValidator.validate()', function () {
    (0, mocha_1.it)('valid results', function () {
        for (const chain in TestCases) {
            for (const c of [chain, ...TestCases[chain].alternatives]) {
                for (const address of addresses_1.default[TestCases[chain].valid]) {
                    valid(address, c);
                }
            }
        }
    });
    (0, mocha_1.it)('invalid results', function () {
        for (const chain in TestCases) {
            for (const c of [chain, ...TestCases[chain].alternatives]) {
                const invalidChains = Object.keys(addresses_1.default)
                    .filter(key => key !== TestCases[chain].valid)
                    .filter(key => { var _a; return !((_a = TestCases[chain].exclude) === null || _a === void 0 ? void 0 : _a.includes(key)); });
                for (const invalidChain of invalidChains) {
                    for (const address of addresses_1.default[invalidChain]) {
                        invalid(address, c);
                    }
                }
            }
...
//             valid('0xdbF03B407c01E7cD3CBea99509d93f8DDDC8C6FB', 'ethereum');
//             valid('0xD1220A0cf47c7B9Be7A2E6BA89F429762e7b9aDb', 'ethereum');
//
...
//             valid('TFEkshkSXo8yMe8vcA6P77XmiLfstNWHyT', 'tron', {  networkType: 'prod' });
//             valid('TUBBzKNM9gr687ucwj8fvVS2Sf2e4WseVa', 'tron', {  networkType: 'testnet' });
//             valid('TFEkshkSXo8yMe8vcA6P77XmiLfstNWHyT', 'tron');
//         });
//
